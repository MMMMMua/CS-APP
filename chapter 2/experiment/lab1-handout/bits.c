...
unsigned float_abs(unsigned uf) {
	unsigned tail = uf & 0x007FFFFFu;
	unsigned expn = uf >> 23 & 0xFFu;
...
int float_f2i(unsigned uf) {	
	...
	if (exp > k || (exp == k && (sign != 0x80000000u || frac != 0)))
		return 0x80000000u;
	...
	if (e <= 23)
		return sig * (p + (frac >> (23 - e)));
	else
		return sig * (p + (frac << (e - 23)));
}
